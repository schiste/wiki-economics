#!/bin/bash
# Observable data loader — scans the data directory and outputs pipeline status as JSON.
# Runs at preview/build time. Refresh the page to re-run.

# Find project root by walking up from the script's location until we hit Cargo.toml
...
# Collect all wiki names across all pipeline stages (unique, sorted)
ALL_WIKIS=""
...
for wiki in $ALL_WIKIS; do
  ...
  if [ -d "$raw_dir" ]; then
    ...
    # Extract dump version from first filename (e.g., "2026-02" from "2026-02.nlwiki.2001.tsv.bz2")
  ...
  if [ -d "$pq_dir" ]; then
    ...
    # Which raw files are not yet ingested?
  ...
  # Computed metrics (per-wiki output)
  ...
  # Dashboard data (site/src/data/<wiki>/)
  dash_dir="$SITE_DATA/$wiki"
  dash_count=0
  if [ -d "$dash_dir" ]; then
    dash_count=$(ls "$dash_dir"/*.parquet 2>/dev/null | wc -l | tr -d ' ')
    printf '      "dashboard": ['
    first_f=true
    for f in "$dash_dir"/*.parquet; do
      [ -f "$f" ] || continue
      $first_f || printf ","
      first_f=false
      name=$(basename "$f" .parquet)
...
# --- Merged output files (in site/src/data/) ---
echo '  "merged": ['
first_file=true
for f in "$SITE_DATA"/*.parquet; do
  [ -f "$f" ] || continue
  $first_file || printf ","
  first_file=false
  name=$(basename "$f" .parquet)
