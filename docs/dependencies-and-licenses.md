...
| DuckDB-WASM | Browser-side analytical engine provided by Observable's client runtime | `MIT` |
...
The repository uses one npm workspace and one root lockfile. Dashboard defaults
are generated by the Rust pipeline, so server-side Node installs do not include
a native DuckDB binding. DuckDB-WASM remains a browser concern for interactive
queries.
