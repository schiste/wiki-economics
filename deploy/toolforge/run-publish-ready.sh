...
report_publication_exit() {
  local status=$?
  if [ "$status" -ne 0 ]; then
    echo "!!! PUBLICATION FAILED run_id=$WIKI_ECON_RUN_ID exit_code=$status log_file=$WIKI_ECON_RUN_LOG_FILE publication_gate=$WIKI_ECON_RUN_PUBLICATION_FILE" >&2
  fi
}
trap report_publication_exit EXIT
