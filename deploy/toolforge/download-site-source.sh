...
usage() {
  echo "Usage: download-site-source.sh <40-character-git-sha> <destination-directory> [workflow-run-id]" >&2
  exit 2
}
...
artifact_name="wiki-econ-site-source-$release_sha"
archive_name="wiki-econ-site-source-$release_sha.tar.gz"
...
for command in gh find; do
  command -v "$command" >/dev/null 2>&1 || {
    echo "Required site-source download command is missing: $command" >&2
    exit 1
  }
done
...
if [ -z "$run_id" ]; then
  ...
  [ -n "$run_id" ] || { echo "No successful completed CI run found for $release_sha" >&2; exit 1; }
fi
...
download_root="$(mktemp -d "$destination/.wiki-econ-site-source-download.XXXXXX")"
...
while IFS= read -r -d '' manifest; do manifests+=("$manifest"); done \
  < <(find "$download_root" -type f -name "$checksum_name" -print0)
[ "${#manifests[@]}" -eq 1 ] || {
  echo "Site-source artifact must contain exactly one $checksum_name; found ${#manifests[@]}" >&2
  exit 1
}

checksum_path=${manifests[0]}
read -r expected_checksum declared_archive < "$checksum_path" || { echo "Cannot read site-source checksum" >&2; exit 1; }
[[ "$expected_checksum" =~ ^[0-9a-f]{64}$ ]] && [ "$declared_archive" = "$archive_name" ] || {
  echo "Site-source checksum manifest is malformed" >&2
  exit 1
}
archive_path="$(dirname "$checksum_path")/$declared_archive"
[ -f "$archive_path" ] && [ ! -L "$archive_path" ] || { echo "Site-source archive is missing" >&2; exit 1; }
...
printf 'site_source_archive=%s\n' "$destination/$archive_name"
printf 'site_source_checksum=%s\n' "$destination/$checksum_name"
